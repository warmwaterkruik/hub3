...
func TestBlank(t *testing.T) {
	...
	for _, tt := range errTests {
		blank, err := NewBlank(tt.input)
		if err != nil && fmt.Sprintf("%v", err) != tt.errString {
			t.Errorf("NewBlank(%q) => %v; want %v", tt.input, err, tt.errString)
		}
		if tt.input != tt.want {
			t.Errorf("NewBlank(%q) => %v; want %v", tt.input, blank.String(), tt.want)
		}
	}
}
